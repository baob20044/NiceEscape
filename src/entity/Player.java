    ...
    public void update(){
        if(keyH.isUpPressed() == true || keyH.isDownPressed() == true ||keyH.isLeftPressed() == true || keyH.isRightPressed() == true){
            ...
            if(gp.getPlayer().isBoatEvent() == true && isCollisionOn() == false){
                switch (direction){
                    case "up" :setWorldY(getWorldY()-getSpeed()*3); support(); break;
                    case "down":setWorldY(getWorldY()+getSpeed()*3); support();break;
                    case "left": setWorldX(getWorldX()-getSpeed()*3);support(); break;
                    case "right":setWorldX(getWorldX()+getSpeed()*3); support();break;
                }
            }
        ...
        if(i!= 999){
            ...
            switch (objectName) {
                case "Key":
                    hasKey++;
                    gp.getObj()[i] = null;
                    gp.getUi().showMassage("You got a key!");
                    break;
                case "Door":
                    gp.getUi().showMassage("You need a key!");
                    if (hasKey > 0) {
                        gp.getObj()[i] = null;
                        hasKey--;
                        gp.getObj()[i] = null;
